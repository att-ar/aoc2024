...
parseLineIntoReport :: BS.ByteString -> [Int]
parseLineIntoReport = mapMaybe readIntBS . BS.words
...
{-
  ...
  print (foldl' sumValidReports 0 (map parseLineIntoReport (BS.lines contents)))
-}
day02P1 :: FilePath -> IO ()
day02P1 filepath = do
  contents <- BS.readFile filepath
  -- convert each line in (BS.line contents) from BS.ByteString to [Int] by applying parseLineIntoReport
  -- Then apply isValidReport which just returns Bool for each report
  -- Then convert to Int using fromEnum
  -- Then sum
  print (sum (map (fromEnum . isValidReport . parseLineIntoReport) (BS.lines contents)))
...
-}
day02P2 :: FilePath -> IO ()
day02P2 filepath = print . sum . map (fromEnum . isFlexibleValidReport . parseLineIntoReport) . BS.lines =<< BS.readFile filepath
